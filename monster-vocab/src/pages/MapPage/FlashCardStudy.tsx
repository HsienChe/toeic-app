...
export default function FlashCardStudy({
  cat, catInfo, customWords, state, onFinish, onBack,
}: FlashCardStudyProps) {
  ...
  if (catWords.length === 0) {
    return (
      <div style={{ padding: 24, textAlign: 'center' }}>
        ...
        <button onClick={onBack} style={btnStyle('#6b7280')}>← 返回</button>
      </div>
    );
  }
  ...
  const STATUS_DOT: Record<string, string> = {
    new: '#9ca3af', learning: '#f59e0b', weak: '#ef4444',
    mastered: '#10b981', fluent: '#8b5cf6',
  };
  const STATUS_LABEL: Record<string, string> = {
    new: '未學習', learning: '學習中', weak: '⚠危險',
    mastered: '精通', fluent: '🌟爛熟',
  };
  ...
  return (
    <div style={{ padding: '16px', paddingBottom: 120, maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: '#1f2937' }}>
            {catInfo.icon} {catInfo.name} — 背誦單字
          </div>
      ...
      <div
        onClick={handleFlip}
        style={{ perspective: '1000px', marginBottom: 20, cursor: 'pointer', userSelect: 'none' }}
      >
        <div style={{
          ...
          <div style={{
            ...cardFaceStyle,
            background: `linear-gradient(135deg, ${catInfo.bg}, #fff)`,
            border: `2px solid ${catInfo.color}40`,
          }}>
            <div style={{
              fontSize: 11, color: catInfo.color, fontWeight: 700,
              marginBottom: 12, letterSpacing: 2, textTransform: 'uppercase',
            }}>
              {catInfo.icon} {catInfo.name}
            </div>
            <div style={{ fontSize: 28, fontWeight: 900, color: '#1f2937', marginBottom: 8, letterSpacing: 1 }}>
              {current.word}
            </div>
            ...
            <div style={{
              fontSize: 12, color: catInfo.color,
          ...
          <div style={{
            ...
            <div style={{ fontSize: 24, fontWeight: 800, color: '#f8fafc', marginBottom: 14 }}>
              {current.meaning}
            </div>
            {current.example && (
              <>
                ...
                <div style={{
                  fontSize: 13, color: '#e2e8f0', fontStyle: 'italic', lineHeight: 1.6,
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        <button
          ...
          style={{
            ...
            background: cardIdx === 0 ? '#f3f4f6' : '#e0e7ff',
            color: cardIdx === 0 ? '#d1d5db' : '#6366f1',
        ...
        <button
          onClick={handleNext}
          disabled={cardIdx === catWords.length - 1}
          style={{
            flex: 1, padding: '12px 0', borderRadius: 12, border: 'none',
            background: cardIdx === catWords.length - 1 ? '#f3f4f6' : '#e0e7ff',
            color: cardIdx === catWords.length - 1 ? '#d1d5db' : '#6366f1',
            fontWeight: 700, fontSize: 15,
            cursor: cardIdx === catWords.length - 1 ? 'not-allowed' : 'pointer',
          }}
        >下一張 →</button>
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {catWords.map((w, i) => {
            ...
            const statusLabel = STATUS_LABEL[wStatus] ?? '';
            return (
              <div
                key={w.id}
                ...
                style={{
                  ...
                  padding: '10px 12px',
                  background: i === cardIdx ? catInfo.color + '12' : '#f9fafb',
                  border: `1.5px solid ${i === cardIdx ? catInfo.color + '50' : '#e5e7eb'}`,
              ...
              >
                ...
                <div style={{
                  ...
                  background: seen.has(w.id) ? '#10b981' : '#e5e7eb',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
                  ...
                  <span style={{ fontSize: 10, color: dotColor, fontWeight: 600 }}>
                    {statusLabel}
                  </span>
                </div>

                {i === cardIdx && (
                  <span style={{ fontSize: 11, color: catInfo.color, fontWeight: 600 }}>查看中</span>
                )}
              </div>
            );
          })}
      ...
      <div style={{
        ...
        background: '#fff', borderTop: '1px solid #f3f4f6', paddingTop: 12,
      }}>
        {!allSeen && (
          <div style={{
        ...
        <button
          ...
          style={{
            ...
            background: allSeen
              ...
              : '#e5e7eb',
            color: allSeen ? '#fff' : '#9ca3af',
            fontSize: 16, fontWeight: 800,
            cursor: allSeen ? 'pointer' : 'not-allowed',
            transition: 'all 0.3s',
            boxShadow: allSeen ? `0 4px 20px ${catInfo.color}50` : 'none',
          }}
        >
...
function btnStyle(color: string): React.CSSProperties {
  return {
    ...
    background: color + '20', color, fontWeight: 700, fontSize: 14, cursor: 'pointer',
  };
}
