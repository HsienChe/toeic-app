...
export default function CatGlossary({
  cat, catInfo, customWords, state, onStudy, onBack,
}: CatGlossaryProps) {
  ...
  const counts = useMemo(() => {
    const c: Record<string, number> = {
      all: catWords.length, new: 0, learning: 0, weak: 0, mastered: 0, fluent: 0,
    };
    catWords.forEach(w => {
      const s = getWordStatus(w.id, masteryMap, fluentSet);
      c[s] = (c[s] || 0) + 1;
    });
    return c;
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          <button
            ...
            style={{
              ...
              cursor: 'pointer', color: '#6b7280', padding: '4px 8px', flexShrink: 0,
            }}
          >←</button>
          <div>
          ...
          {(['new', 'learning', 'weak', 'mastered', 'fluent'] as const).map(s => {
            ...
            return (
              <div
                ...
                style={{
                  background: statusFilter === s ? cfg.bg : '#f9fafb',
                  border: `1.5px solid ${statusFilter === s ? cfg.border : '#e5e7eb'}`,
              ...
              >
                ...
                <div style={{ fontSize: 9, color: '#9ca3af', fontWeight: 500, lineHeight: 1.2 }}>
                  {STATUS_LABEL[s]}
                </div>
              </div>
            );
          ...
          {STATUS_TABS.map(t => (
            <button
              ...
              style={{
                ...
                border: `1.5px solid ${statusFilter === t.id ? catInfo.color : '#e5e7eb'}`,
                fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s',
                background: statusFilter === t.id ? catInfo.color + '15' : '#fff',
                color: statusFilter === t.id ? catInfo.color : '#6b7280',
              }}
            >
              {t.icon} {t.label}
              {t.id !== 'all' && counts[t.id] > 0 && (
                <span style={{
                  ...
                  background: statusFilter === t.id ? catInfo.color : '#e5e7eb',
                  color: statusFilter === t.id ? '#fff' : '#6b7280',
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '48px 0', color: '#9ca3af' }}>
            ...
            <div style={{ fontSize: 14 }}>
              {statusFilter === 'fluent' ? '還沒有滾瓜爛熟的單字' : '此狀態沒有單字'}
            </div>
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    padding: '11px 14px',
                    background: isFluent ? '#faf5ff' : '#fff',
                    border: `1.5px solid ${isFluent ? '#c4b5fd' : '#f0f0f0'}`,
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 15, fontWeight: 700, color: isFluent ? '#6d28d9' : '#1f2937' }}>
                        {w.word}
                      </span>
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            boxShadow: `0 4px 16px ${catInfo.color}40`,
